...
export const useAssetIconCache = () => {
    const cache = useMemo(
        () =>
            new Map<string, { assetIcon: string; chainIcon: string | null }>(),
        [],
    );
    ...
    return (asset: string) => {
        if (!cache.has(asset)) {
            ...
            let mainAsset = lowerAsset
                .replace(/(arb|sol|dot|eth|btc)/, '')
                .trim();
            ...
            const chainIcon = chain
                ? `/icons/${chain}-chain.svg`
                : `/icons/eth-chain.svg`; // Use `-chain` for chains
